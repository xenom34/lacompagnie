...
class CalendarChoice extends React.Component<any, any> {
    ...
    render() {
        let d = new Date();
        return (
            <label id={"calendar" + this.id}
                   className="mdc-text-field mdc-text-field--outlined mdc-text-field--with-leading-icon content">
                ...
                <i id={'calendarIcon' + this.id}
                   className="material-icons mdc-text-field__icon mdc-text-field__icon--leading">event</i>
                <input onChange={this.OnChange} className="mdc-text-field__input" type="date"
                       min={d.toISOString().substring(0, 10)} aria-labelledby="my-label-id"/>
            </label>
        );
    }
}
